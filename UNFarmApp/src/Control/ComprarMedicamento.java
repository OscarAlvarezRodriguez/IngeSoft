...
public class ComprarMedicamento {
    ...
    public String validarDatos(Compra compra, Compramedicamento compramedicamento, short idMed) {
        ...
        if (validarLongitudCantidad(compramedicamento).equals(ci)) {
            return (ci);
        }

        //Cantidad comprada hace que se supere 2000 de stock, entonces retorna ce
        if (validarCantidad(compramedicamento, idMed).equals(ce)) {
            return (ce);
        }

        //Validar el precio unitario
        if (validarPrecioUnitario(compramedicamento.getPreciounitario()).equals(pn)) {
            return (pn);
        }
    ...
    public String validarLongitudCantidad(Compramedicamento compra) {
    ...
    public String validarCantidad(Compramedicamento compra, short idMed) {
        //verificar si compra.getMedicamento().getStock() es una operación válida
        //Si stock disponible mayor a 2000 retorna "ce"
        Medicamento m = medicamentoDao.leer(idMed);
        compra.setMedicamento(m);
        if (compra.getMedicamento().getStock() + compra.getCantidad() > 2000) {
            return (ce);
        }
        return (co);
    }
